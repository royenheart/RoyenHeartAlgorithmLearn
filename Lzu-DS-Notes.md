...
2.1		主要术语、概念：
2.1.1		数据结构：
		文字概念：给定数据对象及其上面定义的操作（运算）所形成的系统
		数学描述：Data Structure = (Class，Operation)
			**给定：即具体化，限定对象的范围和性质
			**操作：处理的需求，即运算
		研究内容：
			各数据元素的性质和之间的逻辑关系以及数据元素在计算机中的存放方式（物理关系）
				**逻辑关系：对数据间关系的人为理解和解释，侧重人的理解
				**物理关系：对数据的运算（操作），数据的运算关系
			总的来说分为：逻辑关系、存储关系、运算关系
			逻辑关系（结构）：
				自然形态：数据元素之间的一种联系
				**联系：事物的关联方式或邻接关系。反映人对数据、现实的理解
				逻辑结构：联系的集合
					概念：数据元素之间的关系的描述
					文字描述：数据元素之间逻辑关系的集合
					数学描述：B = (K，R) {K：数据元素的有限集合R：数据元素关系集合(K->K，即<K，K>)
				逻辑关系(结构)分类：
					1、线性关系（结构），1对1，线性
			...
			存储关系（结构）
				概念：在计算机存储空间下，数据元素逻辑关系在存储器中的体现。即数据元素在存储器中的联系
				文字描述：存储关系的集合
				计算机中实现：映射关系S，即数据对象K根据S映射关系投影到计算机存储的集合M，存储的对象即对象K的像
				**映射关系：是逻辑关系在计算机中的表现，表现现实中的逻辑关系
				存储关系（结构）分类：
					即根据映射关系S的不同进行区分
					1、顺序关系（结构），存储器中对应内容的存储地址连续分布
					构造方法：在存储区中按连续地址空间顺序依次存放数据元素
					2、链接关系（结构），使用指针，实际内容分布离散
					构造方法：结点存储单元分两部分，一部分存储结点本身信息（数据项），另一部分存放该节点关联的其他结点的地址信息（指针项）
					即结点（数据元素）再加入指针数据项
					**指针：存储指向元素在存储空间中分配的空间的地址的信息（值）
					3、索引关系（结构）
					构造方法：索引区+数据区。数据区—所有存储结点连续存放；索引区—索引指向的存储节点
					4、散列关系（结构），Hash结构为代表
					构造方法：根据结点关键字值，由一定函数关系（f）确定对应数据元素的存储地址
					数学描述：取K（数据元素，结点，记录）的某一数据项或多个数据项，构成关键字Wik，K对应的存储地址由函数loc（K） = f（Wik）
			运算关系：
				定义：定义在逻辑关系上，其具体实现需要根据存储结构
				运算集合：有增、删、改、查等运算
2.1.2			算法：
			运算的方法：问题的一个具体解决方案，即将输入运算到输出的过程
			**问题：对于计算而言，是一个需要完成的任务，即对应一组输入至一组输出的过程
			定义：
				算法是解决某个问题的指令的有限集
				准则：有穷性，确定性，可行性，输入输出准则（至少一个输入，可以没有输出）				
				准则概念：
				确定性：算法的每一步必须是明确定义，对每一个情况，执行行动需做明确规定
				可行性（有效性）：算法可行，每条指令应由指令集中（已经实现）的指令构成
			内涵与分类：
				内涵：是解决问题的策略机制，解决问题过程的完整描述
				**过程：事物发展或事物进行所经过的程序即次序，基于时间表征
				**根据信息处理要求和目的性，过程可分为一般过程和函数过程
				**一般过程：强调计算的过程本身
				**函数过程：强调计算的“值”，即结果
				基于计算的内涵：算法是通过时间表征的指令序列，整个算法是时间上的状态序列（本源性）
			算法与程序的异同：
				算法能解决问题，有具体步骤的方法（过程），是解决问题的过程
				程序是算法（思想）的具体实现，即在计算机程序设计语言中的实现。是思想的具体表现
				程序具有有穷性，表示能在有限时间内完成
				在描述形式上：
				算法是概念语言，用以描述解决问题的思想，是抽象化的运算过程
				程序需借助特定的程序设计语言进行描述，是对解决问题思想的具体化描述
2.2		主要模型、原理、方法：
2.2.1			数据结构的选择与评价：
			评价标准：
			...
			算法描述和设计原则
				算法常见描述形式：
					1、具体计算机程序设计语言
					2、自然语言，基于自然界的语言。但具有二义性，难以完全精确表达。可以进行快速描述
					3、PDL语言，伪程序设计原因呢，或伪码语言
			...
			算法分析及有效算法
				概述：算法分析是一种目的，用于比较不同算法并选取对对应情况相对高效的算法进行问题的解决
				概念：
				时间复杂度：利用某算法处理问题规模为n的输入所需要的时间，其用数学语言表示为T（n）
				空间复杂度：利用莫算法处理问题规模为n的输入所需要花费的空间（对于计算机的存储空间而言），其用数学语言表示为S（n）
			算法复杂性阶：
				概念：指对不同算法的时间和空间复杂度不已精确值计算，仅考虑其所在的档次即位阶
				定义：对一正常数C，若算法能在Cn^2时间内处理问题规模为n的问题，则此算法的时间复杂度为O（n ^2），即n^2阶或与n^2同阶
				分类（部分）：O（2^n），算法与2^n同阶，为指数时间；O（n^k），算法与n^k同阶，多项式时间
				有效算法：在通常情况下以多项式时间为界阶的算法，在某些特定问题或者领域上可能有所不同
				难解性问题：不存在多项式时间为界限的算法可以进行解决，则称这个算法是难解的
				算法的分析：
					最坏情况下的时间/空间复杂度
					平均情况下的时间/空间复杂度
					通常考虑上述两个参数，选取对特定问题较为合适的算法
					也可以考虑复杂度的加权平均特性
					原因：规模为n的输入在不同情况下出现的概率是不同的
					实现困难：1、输入量分布要进行假设量化2、实现起来也一般考虑算术平均 
				时间复杂性分析：
					1、根据该类问题特点合理选择一种或者n种操作作为标准操作即基础操作
					2、确定每个算法在给定输入下总执行的标准操作次数，根据次数和问题规模推导得出时间函数
					3、确定函数的阶，即次数的最大项
				空间复杂度分析：
					在标准存储上，比较算法所需要的附加存储数据复杂度
			算法设计方法：
				概述：采用系统科学观点
				方法：自顶而下，逐步求精
				顶：表述问题的综合以及问题抽象全貌
			...
			PDL描述算法：
				...
				PDL书写格式/要求：
					...
					词的定义及说明：
					计算机语言中的词汇：标识符
					**标识符：语言中的字符按一定规则形成具有特定含义的词
					PDL的命名规则
					词：1、一个以字母开始的字母数字集2、词能传递出有关信息及意义且是对人而言
					标识符：分为过程名和变量/常量名，过程名调用前需及逆行定义；变量/常量名调用前需说明，即给出实际的意义
					常量说明：CONST 常量名=常量值
					变量说明：VAR 变量名表：数据类型名
						**常见数据类型：
						**BOOLEAN：布尔型
					...
					构造类型：从基本类型中构造
						...
						下标类型一般使用子介型，一个下标符号对应一个内容
						记录型：P = RECORD
										域标识符1：类型1；
										……
										域标识符2：类型2；
									END;
						指针型：TYPE A = ↑类型名；{定义一个新指针类型}
								VAR A：↑类型名
				基本语句语法：
					赋值语句：为符号、语句赋予实际含义
						对计算机：能够解释并执行处理
					...
					格式：变量名表达式
					**变量名：一个符号，赋予其含义表示具体事物
					**表达式：具有具体事物的含义和意义
					条件语句：选择语句，基于一定条件进行指令，语句的选择							对计算机：根据不同条件选定不同的方法（过程）
						格式：IF 条件 THEN 语句组
						条件1：IF 条件THEN 语句组
						条件2：IF 条件 THEN 语句组1
									    ELSE 语句组2
...
5.3		主要算法：
5.3.1			朴素字符串匹配算法（布鲁特夫斯算法）： 
			即一个字符一个字符进行比较查找，每次对模式移动一位进行比较
5.3.2			KMP算法（字符串模式匹配算法）：
